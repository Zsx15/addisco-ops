...
_ERROR_COLORS = ["#dc2626", "#b45309", "#d97706", "#0369a1", "#7c3aed"]


def render_score_chart(df_evo: pd.DataFrame) -> None:
    if df_evo.empty or "score" not in df_evo.columns:
        st.caption("Données insuffisantes pour afficher la progression.")
        return
    df_plot = df_evo[["score"]].rename(columns={"score": "Score"})
    st.line_chart(df_plot, height=160, use_container_width=True)


def render_error_heatmap(df_errors: pd.DataFrame) -> None:
    """Distribution des erreurs — barres colorées par type."""
    if df_errors.empty or "error_type" not in df_errors.columns:
        st.caption("Aucune erreur enregistrée.")
        return
    ...
    for i, (_, row) in enumerate(df_errors.iterrows()):
        ...
        rows_html += (
            ...
            f'<div style="font-size:11px;color:#475569;min-width:130px;flex-shrink:0">'
            f'{label}</div>'
            ...
            f'</div>'
            f'<div style="font-size:11px;color:{color};font-weight:700;min-width:24px;'
            f'text-align:right">{count}</div>'
            f'</div>'
        )

    st.markdown(
        f'<div style="padding:10px 14px;background:#f8fafc;border:1px solid #e2e8f0;'
